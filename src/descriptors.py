...
def shortest_path_descriptors(graph,filename,black_vertices,white_vertices, dim, shortest_path_to_red, \
    ...
    """
        ...
        Args:
            ...
            black_vertices (list): List of indices of ‘black’ vertices.
            white_vertices (list): List of indices of ‘white’ vertices.
            dim (float): Dimensionality constant used in the calculation of tolerance.
            shortest_path_to_red (list): List containing shortest path distances from each vertex to the nearest ‘red’ metavertex.
            shortest_path_to_blue (list): List containing shortest path distances from each vertex to the nearest ‘blue’ metavertex.
            countBlack_Red_conn (int): The total number of ‘black’ vertices in connected components that connect to the top.
            countWhite_Blue_conn (int): The total number of ‘white’ vertices in connected components that connect to the bottom.
...
def descriptors(graph,filename,black_vertices,white_vertices, black_green,black_interface_red, white_interface_blue, \
    ...
    This function computes all the descriptors for the graph given and saves them  in a dictionary.
     Most of these inputs are generated by the ‘generateGraph’ function then passed into the ‘descriptors’ function in order to generate all the descriptors from the given data.
    ...
    Args:
        ...
        black_vertices (list): List of indices of ‘black’ vertices.
        white_vertices (list): List of indices of ‘white’ vertices.
        black_green (int): The number of edges connecting a ‘black’ vertices adjacent to a ‘green’/ interface vertex.
        black_interface_red (int): The number of ‘black’ vertices with a path to the ‘red’ metavertex/anode (CT_e_D_An).
        white_interface_blue (int): The number of ‘white’ vertices with a path to the ‘blue’ metavertex/cathode (CT_e_A_Ca).
        dim (float): Dimensionality - the constant used in the calculation of tolerance.
        interface_edge_comp_paths (int): The number of interface edges with complementary paths (CT_e_conn).
        shortest_path_to_red (list): List containing shortest path distances from each ‘black’ vertex to the ‘red’ metavertex.
        shortest_path_to_blue (list): List containing shortest path distances from each ‘white’ vertex to the ‘blue’ metavertex.
        CT_n_D_adj_An (int): The number of ‘black’ vertices in direct contact with the top, or anode, or ‘red’ metavertex.
        CT_n_A_adj_Ca (int): The number of ‘white’ vertices in direct contact with the bottom, or cathode, or ‘blue’ metavertex.
