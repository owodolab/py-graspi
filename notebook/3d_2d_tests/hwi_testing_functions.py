...
def plot_total_execution_and_memory(csv_file):
    """
    Plot total execution time and peak memory usage from a CSV file.
    Each pattern group gets its own plot.
    """
    ...
    for pattern, files in pattern_groups.items():
        ...
        ax1.plot(subset['Test File'], subset['total_execution_time'], marker='o', linestyle='-', color='blue')
        ...
        ax2.plot(subset['Test File'], subset['peak_memory_usage'], marker='s', linestyle='-', color='green')
...
def plot_stepwise_execution_and_memory(csv_file):
    """
    Plot separate bar charts for execution time and memory usage
    of Graph and Descriptor steps for each file.
    """
    df = pd.read_csv(csv_file)
    pattern_groups = group_files_by_pattern(df['Test File'])
...
def plot_total_execution_and_memory_compare(csv_file1, csv_file2):
    ...
    for pattern, files in pattern_groups.items():
        ...
        ax1 = plt.subplot(2, 1, 1)
        ax1.plot(sorted_test_files, subset1['total_execution_time'], marker='o', linestyle='-', color='blue', label='Run 1')
        ax1.plot(sorted_test_files, subset2['total_execution_time'], marker='o', linestyle='-', color='red', label='Run 2')
        plt.xlabel('Test Files')
        plt.ylabel('Time (s)')
        ...
        ax2 = plt.subplot(2, 1, 2)
        ax2.plot(sorted_test_files, subset1['peak_memory_usage'], marker='s', linestyle='-', color='green', label='Run 1')
        ax2.plot(sorted_test_files, subset2['peak_memory_usage'], marker='s', linestyle='-', color='purple', label='Run 2')
        plt.xlabel('Test Files')
        plt.ylabel('Memory Usage (bytes)')
...
def plot_stepwise_execution_and_memory_compare(csv_file1, csv_file2):
    ...
    for pattern, files in pattern_groups.items():
        ...
        for i, ((_, row1), (_, row2)) in enumerate(zip(subset1.iterrows(), subset2.iterrows())):
            ...
            ax_time.bar(x - 0.2, times1, bar_width, color=['blue', 'red'], label='Run 1')
            ax_time.bar(x + 0.2, times2, bar_width, color=['cyan', 'orange'], label='Run 2')
            ax_time.set_xticks(x)
            ax_time.set_xticklabels(['Graph', 'Descriptor'])
            ...
            ax_mem.bar(x - 0.2, mems1, bar_width, color=['green', 'purple'], label='Run 1')
            ax_mem.bar(x + 0.2, mems2, bar_width, color=['lightgreen', 'pink'], label='Run 2')
            ax_mem.set_xticks(x)
            ax_mem.set_xticklabels(['Graph', 'Descriptor'])
